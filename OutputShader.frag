...
void main() {
    ...
    // Проверка, находится ли текущая точка внутри треугольника
    float d1 = sign((uv - v0.xyx) * (v1 - v0).yxw);
    float d2 = sign((uv - v1.xyx) * (v2 - v1).yxw);
    float d3 = sign((uv - v2.xyx) * (v0 - v2).yxw);
    bool isInside = (d1 == d2) && (d2 == d3);

    // Если текущая точка внутри треугольника, установите цвет на красный, иначе используйте цвет из вашего шейдера
    vec3 triangleColor = isInside ? vec3(1.0, 0.0, 0.0) : col;
